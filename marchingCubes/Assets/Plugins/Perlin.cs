...
public class FractalNoise
{
	...
	public float HybridMultifractal(float x, float y, float z, float offset)
	{
		float weight, signal, remainder, result;
		
		result = (m_Noise.Noise (x, y, z)+offset) * m_Exponent[0];
		weight = result;
		x *= m_Lacunarity; 
		y *= m_Lacunarity;
		z *= m_Lacunarity;
		...
		{
			if (weight > 1.0F) weight = 1.0F;
			signal = (m_Noise.Noise (x, y, z) + offset) * m_Exponent[i];
			result += weight * signal;
			weight *= signal;
			x *= m_Lacunarity; 
			y *= m_Lacunarity;
		...
		remainder = m_Octaves - m_IntOctaves;
		result += remainder * m_Noise.Noise (x,y,z) * m_Exponent[i];
	...
	public float RidgedMultifractal (float x, float y, float z, float offset, float gain)
	{
		float weight, signal, result;
		int i;
	...
	public float BrownianMotion (float x, float y, float z)
	{
		float value, remainder;
		long i;
		...
		for (i=0;i<m_IntOctaves;i++)
		{
			value = m_Noise.Noise (x,y,z) * m_Exponent[i];
			x *= m_Lacunarity;
			y *= m_Lacunarity;
			z *= m_Lacunarity;
		}
		remainder = m_Octaves - m_IntOctaves;
		value += remainder * m_Noise.Noise (x,y,z) * m_Exponent[i];
